...
Grafo* criarGrafo(int maxVertices) {
    ...
    for (int i = 0; i < maxVertices; i++) {
        ...
        // Inicializa campos da sala para evitar lixo
        g->salas[i].ativo = 0;
        g->salas[i].temItem = 0;
...
void carregarMapaPadrao(Grafo *g) {
    g->numVertices = 0;
    // Limpa matriz e inicializa campos de salas
    ...
    s=&g->salas[0]; s->id=0; s->ativo=1; strcpy(s->nome, "Cela 402"); strcpy(s->descricao, "Sua cela escura."); s->temItem=0; s->precisaItem=0;
    s=&g->salas[1]; s->id=1; s->ativo=1; strcpy(s->nome, "Corredor"); strcpy(s->descricao, "Corredor da morte."); s->temItem=0; s->precisaItem=0;
    s=&g->salas[2]; s->id=2; s->ativo=1; strcpy(s->nome, "Enfermaria"); strcpy(s->descricao, "Cheiro de álcool."); s->temItem=0; s->precisaItem=0;
    s=&g->salas[3]; s->id=3; s->ativo=1; strcpy(s->nome, "Pátio Central"); strcpy(s->descricao, "Área aberta chuvosa."); s->temItem=0; s->precisaItem=0;
    s=&g->salas[4]; s->id=4; s->ativo=1; strcpy(s->nome, "Escritório"); strcpy(s->descricao, "Tem uma chave na mesa!"); s->temItem=1; s->precisaItem=0;
    s=&g->salas[5]; s->id=5; s->ativo=1; strcpy(s->nome, "Refeitório"); strcpy(s->descricao, "Comida podre."); s->temItem=0; s->precisaItem=0;
    s=&g->salas[6]; s->id=6; s->ativo=1; strcpy(s->nome, "Controle"); strcpy(s->descricao, "Requer chave."); s->temItem=0; s->precisaItem=1;
    s=&g->salas[7]; s->id=7; s->ativo=1; strcpy(s->nome, "SAÍDA"); strcpy(s->descricao, "Liberdade!"); s->temItem=0; s->precisaItem=0;
...
void salvarGrafoArquivo(Grafo *g) {
    ...
    for(int i=0; i<g->numVertices; i++)
        fprintf(f, "%d;%s;%s;%d;%d\n", g->salas[i].id, g->salas[i].nome, g->salas[i].descricao, g->salas[i].temItem, g->salas[i].precisaItem);
...
void carregarGrafoArquivo(Grafo **g) {
    ...
    int qtd; if (fscanf(f, "%d\n", &qtd) != 1) { fclose(f); printf("Arquivo corrompido.\n"); return; }

    *g = criarGrafo(20);
    (*g)->numVertices = qtd;

    for(int i=0; i<qtd; i++) {
        // Agora lemos temItem e precisaItem também
        int id, temItem, precisaItem;
        char nome[MAX_BUFFER], desc[MAX_BUFFER];
        fscanf(f, "%d;%[^;];%[^;];%d;%d\n", &id, nome, desc, &temItem, &precisaItem);
        (*g)->salas[i].id = id;
        strcpy((*g)->salas[i].nome, nome);
        strcpy((*g)->salas[i].descricao, desc);
        (*g)->salas[i].ativo = 1;
        (*g)->salas[i].temItem = temItem;
        (*g)->salas[i].precisaItem = precisaItem;
    }
    int u, v;
    while(fscanf(f, "%d %d\n", &u, &v) != EOF) {
        if (u >= 0 && u < (*g)->numVertices && v >= 0 && v < (*g)->numVertices) {
...
// --- MODO RPG IMPLEMENTAÇÃO ---
void jogar(Grafo *g) {
    if (g->numVertices == 0) {
        printf("\nCarregue o MAPA PADRÃO antes de jogar!\n");
        return;
    }

    int pos = 0;            // Começa na cela 402 (assumindo que o mapa padrão tem cela 0)
    int temChave = 0;       // Jogador não começa com a chave
    int op;

    printf("\n======= INÍCIO DO JOGO =======\n");
    printf("Você acorda na %s...\n", g->salas[pos].nome);

    while (1) {
        printf("\n---------------------------------\n");
        printf("Local atual: %s\n", g->salas[pos].nome);
        printf("%s\n", g->salas[pos].descricao);

        // Coletar item, se existir
        if (g->salas[pos].temItem == 1) {
            printf("\nVocê encontrou um ITEM: CHAVE!\n");
            printf("Chave coletada!\n");
            temChave = 1;
            g->salas[pos].temItem = 0; // Chave não reaparece
        }

        // Vitória
        if (strcmp(g->salas[pos].nome, "SAÍDA") == 0) {
            printf("\n*********************************\n");
            printf(" PARABÉNS! VOCÊ ESCAPOU DA PRISÃO!\n");
            printf("*********************************\n");
            break;
        }
        ...
        printf("\nO que fazer?\n");
        printf("1. Mover para outra sala\n");
        printf("0. Encerrar jogo\n");
        printf("Opção: ");
        if (scanf("%d", &op) != 1) { // proteção contra input inválido
        ...
        if (op == 0) {
            ...
            int destino;
            printf("ID da sala destino: ");
            if (scanf("%d", &destino) != 1) { while (getchar() != '\n'); printf("Entrada inválida.\n"); continue; }

            if (destino < 0 || destino >= g->numVertices) {
                printf("ID inválido.\n");
                continue;
            }

            if (g->adj[pos][destino] == 0) {
                printf("Não há passagem para essa sala.\n");
                continue;
            }

            // Porta trancada
            if (g->salas[destino].precisaItem == 1 && !temChave) {
                printf("\nA porta está trancada! Você precisa de uma CHAVE.\n");
                continue;
            }

            printf("Você se move para %s...\n", g->salas[destino].nome);
            pos = destino;
        } else {
...
void adicionarSalaTeste(Grafo *g, char *nome) {
    ...
    g->salas[id].temItem = 0;
    g->salas[id].precisaItem = 0;
    g->numVertices++;
}
